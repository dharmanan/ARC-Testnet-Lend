...
**Status**: ✅ DOCUMENTED
**Severity**: 🟡 Minor (UX delay, not functional issue)
**Impact**: Users may be confused, but transactions complete normally
**Fix**: Wait 5-10 seconds, proceed as normal
...
3. MetaMask receives transaction to sign
   ↓
4. MetaMask attempts to verify data:
   a. Contacts RPC provider (blockdaemon)
   b. Asks: "What's the native token balance?"
   c. Waits for response...
   ↓
5. RPC is slow (5-10 sec delay) ⏳
   ↓
6. MetaMask: "Hmm, haven't heard back yet"
   ↓
7. MetaMask shows RED ALERT:
   "Unknown method or Failed to retrieve metadata"
   ↓
8. 5-10 seconds later...
   ↓
9. RPC responds ✓
...
2. Fetch metadata from RPC
   const metadata = await rpc.getTransactionMetadata(tx)
   
3. Wait for response...
   ⏳ 5-10 seconds
   
4. If no response within 5 sec:
   → Show red warning: "Failed to retrieve"
...
└────────────────┬────────────────────────────────────────┘
    ...
    └────────┬───────────────────────┘
             ...
             ↓ (T=1-2s)
    ┌────────────────────────────────┐
    │ Shows RED ALERT (still waiting │
    │ for RPC response)              │
    └────────┬───────────────────────┘
             │
             ↓ (T=5-10s) ⏳
    ┌────────────────────────────────┐
    │ RPC finally responds ✓         │
    │ MetaMask receives:             │
    │ - Native balance verified      │
    │ - Transaction valid            │
    │ - Ready to sign                │
    └────────┬───────────────────────┘
             │
...
| 0s | User clicks Swap | ✓ Initiated |
| 0-1s | MetaMask receives TX | ⏳ Processing |
| 1-2s | MetaMask queries RPC | ⏳ Waiting |
| 2-5s | **RED ALERT APPEARS** | 🔴 **Warning** |
| 5-10s | RPC responds slowly | ⏳ Still waiting |
| 10s | RPC data received | ✓ Ready |
| 10s+ | **RED ALERT DISAPPEARS** | ✅ **Safe** |
| 10s+ | "Sign" button ready | ✓ Ready to proceed |
| 11s | User clicks "Sign" | ✓ Confirmed |
| 12s+ | Approval complete | ✓ Success |
...
### Step 3: Watch For Alert to Disappear ✓
```
After 5-10 seconds:
- Red alert will DISAPPEAR
- Data will appear correctly
- "Sign" button becomes available
```

### Step 4: Click "Sign" ✓
```
Once alert is gone and data looks correct:
- Click "Sign"
- Approve the transaction
- Swap proceeds normally
```
...
```
RPC Provider: blockdaemon
Latency: ~2-3 seconds normal
During peaks: ~5-10 seconds

MetaMask timeout: ~5 seconds

What happens:
- Request sent at T=0
- MetaMask times out at T=5
- Response arrives at T=7-8
- MetaMask updates UI at T=7-8
```

### ERC20 Approval Verification

```javascript
// MetaMask needs to verify:

1. Token contract exists:
   await eth_call(0x27488..., balanceOf(user))
   
2. Token is valid ERC20:
   await eth_call(0x27488..., decimals())
   
3. Pair contract is valid:
   await eth_call(0x27e14..., codeHash)
   
4. Network matches:
   chainId === '0x4cf1a2'

All these calls go through RPC.
RPC is slow (testnet) → red alert appears.
```
...
| Platform | Red Alert? | Cause | Solution |
|----------|-----------|-------|----------|
| Uniswap | ✓ Sometimes | RPC delay | Wait 5-10s |
| Curve | ✓ Sometimes | RPC delay | Wait 5-10s |
| 1inch | ✓ Sometimes | RPC delay | Wait 5-10s |
| Aave | ✓ Sometimes | RPC delay | Wait 5-10s |
| ARC (Testnet) | ✓ Sometimes | RPC delay | Wait 5-10s |
...
// Option 2: Retry logic with fallback
primary RPC → fallback RPC
// Pros: Better reliability
// Cons: More complex code

// Option 3: Prefetch data
Start verifying when swap form opens
// Pros: Ready before user signs
// Cons: Uses network bandwidth

// Option 4: Custom UI notification
...
- ✓ Fast enough (2-3s normal)
- ✓ Well-maintained
- ⚠️ Occasional delays during peak
...
Blockchain Node (RPC Provider)
    ↑ (JSON Response)
User receives: Token balance, network status, etc.
```
...
During swap:
Mainnet: Red alert gone in 1s ✓
Testnet: Red alert shows for 5-10s ⏳
```
...
**"The red alert in MetaMask during swap approval is caused by the testnet RPC being slow to verify the native token balance. This is completely normal and happens on all blockchains. Just wait 5-10 seconds and the alert will disappear. Then click Sign to proceed with confidence."**
...
**Q: Is the red alert a security issue?**
A: No, it's a UX issue. MetaMask is just slow to verify. The transaction is fine.
...
**User says**: "Red alert appears, is this a scam?"
**Response**: "No, it's the testnet RPC being slow. Wait 5-10 seconds and it will disappear. Then click Sign. This happens on all blockchains. Completely safe."

**User says**: "Should I click Sign while red?"
**Response**: "You can, but wait for it to clear for better UX. It won't matter either way, but waiting is safer for user confidence."

**User says**: "What if I've been waiting 10 minutes?"
**Response**: "Try refreshing the page. If it persists, try a different browser or restart MetaMask. Might be a connection issue."
...
- `QUICK_REFERENCE_RED_ALERT.md` - Quick 2-minute answer
- `WBTC_ARC_APPROVAL_EXPLAINED.md` - Why MetaMask shows "Withdrawal request"
